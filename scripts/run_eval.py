...
def response_samples(resp: Any) -> list[Any]:
    for attr in ('samples', 'outputs', 'responses'):
...
def main() -> int:
    ...
    if args.model_path:
        print(f'creating sampler from model_path: {args.model_path}')
        sampler=service.create_sampling_client(model_path=args.model_path)
    else:
        print(f'creating sampler from base_model: {args.base_model}')
    ...
    with out_path.open('w', encoding='utf-8') as f:
        for scenario in scenarios:
            ...
            f.write(json.dumps(rec, ensure_ascii=False) + '\n')
            print(f"sampled {scenario['id']}: {len(text)} chars")
